...
from drwatson_core import print_imperative, print_info, request_input
...
if len(sys.argv) <= 1:
    drwatson_core.fatal('Required arguments: <can-iface>')

can_iface_name = sys.argv[1]

drwatson_core.show_legend()
...
drwatson_core.execute_shell_command('ip link set %s up type can bitrate 100000 sample-point 0.875',
                                    can_iface_name, ignore_failure=True)

def process_one_device():
    drwatson_core.execute_shell_command('ifconfig %s down && ifconfig %s up', can_iface_name, can_iface_name)
